...
io.on("connection", (socket) => {
  ...
  socket.on("specialMoveCall", ({ room_id, tournamentId, move }) => {
    io.to(room_id).emit("specialMoveReceive", {specialMove: move});
  });
  ...
  socket.on("player_ready", async ({ room_id, username, tournamentId }) => {
    ...
    try {
      ...
      if (playersDbs.success) {
        ...
        const newRooms = createRound(playersDbs.players, tournamentId);
          ...
          if (newRooms.success) {    
            ...
            [room.playerone, room.playertwo].forEach((player) => {
              const newRoom = newRooms.rooms.find(
